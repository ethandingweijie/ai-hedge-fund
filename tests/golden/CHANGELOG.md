...
## 2026-09-18T14:19:27+00:00

- regenerated at HEAD: `4ad14a7`
- fixtures recorded at: `30b26702d3c786e1835dd8e5cc629191e3d75c95`
- tickers: 14
- tolerance: ±5% on numeric leaves
- reason: Phase 1.4 telemetry: quarterly balance-sheet overlay now records the size of the move it made (BALANCE_SHEET_QUARTERLY_STEP_CHANGE, applied: False, 25% owner-set threshold). Disclosure only -- no valuation moved.

## 2026-09-18T14:40:47+00:00

- regenerated at HEAD: `4ad14a7`
- fixtures recorded at: `30b26702d3c786e1835dd8e5cc629191e3d75c95`
- tickers: 14
- tolerance: ±5% on numeric leaves
- reason: Phase 1.4 telemetry, applied: False. The quarterly balance-sheet overlay now records the size of the move it made, as GATE_BALANCE_SHEET_QUARTERLY_STEP_CHANGE / metric balance_sheet_quarterly_step_change, carrying the owner's six keys verbatim (flag, annual_net_cash, quarterly_net_cash, delta_ratio rounded to 4dp, source=quarterly_overlay_refresh, action=applied_quarterly_override) plus one addition that is mine and is disclosed as such in the source comment: currency_basis=source_currency_pre_fx, because both operands are computed before the downstream FX loop and a reader comparing them against a reported-currency disclosure would otherwise be off by the FX rate with nothing on the payload to say so. Threshold 0.25, owner-set 2026-09-18 replacing the plan's 15%; denominator max(abs(annual_net_cash), 1.0), the annual figure only, floored at one currency unit; comparison strictly greater than. This entry SUPERSEDES the terse 14:19:27 entry above it, which named no move set; the projections in it and in this one are identical. EXPECTED MOVES, NAMED BEFORE REGENERATING AND CONFIRMED BY AN INDEPENDENT NUMERIC DIFF OF THE WHOLE SNAPSHOT DOCUMENT: NUMERIC LEAVES THAT MOVED = 0. All fourteen base intrinsic values are byte-identical (02888_HK, 09988_HK 160.84, AAPL 219.8, BABA 193.13, BN4_SI, C38U_SI, COST 1324.57, D05_SI, FCX, MELI 5578.42, MU 293.14, SCHW, U96_SI 5.73, V 428.47) and no weight, method leg, wacc, fx_rate, net_debt, shares_outstanding or 12m target moved anywhere. NON-NUMERIC FIELDS CHANGED = 35, on EIGHT fixtures only (09988_HK, AAPL, BABA, COST, MELI, MU, U96_SI, V), four fields each: gate_metrics gains 'balance_sheet_quarterly_step_change', and scenarios.bear.forward_flags / scenarios.base.forward_flags / scenarios.bull.forward_flags each gain one prose line. The three scenario lists move together because the lift appends to ticker_forward_flags, which the scenario loop copies per scenario -- that is the same channel the existing balance-sheet overlay prose already travels on, so the flag inherits an established path rather than opening one. The remaining six fixtures (02888_HK, BN4_SI, C38U_SI, D05_SI, FCX, SCHW) are quiet because their replayed annual and quarterly net cash differ by less than 25% or the overlay did not substitute a period. WHY A PURE applied:False FLAG STILL MOVES THE BASELINE: golden_replay.py:419-426 projects gate_evaluations down to sorted METRIC NAMES (gate_metrics) rather than snapshotting payloads, so any new gate -- including one that changes no number -- adds a name to the projection. That is a known property of the projection, not evidence of a valuation move, and the numeric diff above is what rules the latter out. CALIBRATION FINDING, RECORDED HERE RATHER THAN TUNED AROUND: the flag fires on 8 of 14 fixtures (57%) with delta_ratio min/median/max 0.4620 / 0.6416 / 4.9521. A dedicated probe (scratchpad/probe_stepchange_golden.py, one subprocess per fixture, PROBE_RC=0) was written because the golden diff cannot answer whether those are real moves -- the projection carries a gate's NAME and never its MAGNITUDE. It found 0 of 8 firings had a zero annual base, so none is a trivial firing off the max(abs(annual), 1.0) denominator floor; every one is a genuine >25% net-cash move. The reason the rate is nonetheless high is that the comparison is NOT quarter-over-quarter: it spans annual-year-end to latest-quarter, which is up to NINE MONTHS (AAPL 2025-09-27 to 2026-06-27, MU 2025-08-28 to 2026-05-28, V 2025-09-30 to 2026-06-30, COST 2025-08-31 to 2026-05-10) against six months for MELI and U96_SI and three for 09988_HK and BABA. The owner's stated rationale for 25% was to clear standard quarterly working-capital drift, which is a one-quarter concept. Left as the owner set it: the threshold is theirs, the record carries delta_ratio so the size of every move is visible, and a disclosure-only flag that fires on most runs still delivers exactly what it was asked for. Whether 57% is the intent, or whether the ratio should be normalised per elapsed quarter, is an owner decision and is not made here. ALSO IN THIS BASELINE, PRODUCING ZERO MOVES BY CONSTRUCTION: the Phase 1.4 provider half -- HK short_term_investments aggregated from four EastMoney labels, SG short_term_investments by corrected precedence with a coherence guard, the cash/cash_and_equivalents alias, and net_debt derivation where yfinance reports nothing. Golden fixtures record at the FMP HTTP layer only (raw/calls.json carries no AKShare and no yfinance targets), so during replay try_fmp succeeds and the HK/SG fallbacks never execute. Verified by measurement, not by argument: the overlay itself DOES run in replay because search_line_items(period='quarterly') resolves through the recorded _fmp_get. No Phase 1.4 provider figure is pinned by this baseline; tests/test_phase14_balance_sheet_fallbacks_0918.py (55 tests) pins them structurally and against the measured live tables instead.

## 2026-09-18T19:01:31+00:00

- regenerated at HEAD: `15d4e08`
- fixtures recorded at: `30b26702d3c786e1835dd8e5cc629191e3d75c95`
- tickers: 14
- tolerance: ±5% on numeric leaves
- reason: Owner ruling 2026-09-19 on the step-change telemetry, verbatim: "Adjust the step-change flag threshold from 0.25 to 0.50 (50%) when period_delta_days > 180 (or leave it at 0.50 globally for the annual-to-quarterly comparison). Keep it strictly as telemetry-only (applied: False) as designed." The CONDITIONAL shipped rather than the parenthetical global alternative, and that choice is mine and is recorded as mine in the source comment: a genuine 91-day year-end-to-Q1 comparison keeps the tighter 0.25 and stays sensitive, while the multi-quarter case is the one that needs the wider band. 180 itself is not "> 180", so a 180-day gap stays strict; both operators are pinned by test.

WHY THE RULING WAS MADE, MEASURED NOT ARGUED. At a flat 0.25 the flag fired on 8 of the 8 golden fixtures whose quarterly overlay applied -- a 100% firing rate, so the flag carried no information. The cause was neither structural moves nor the max(abs(annual_net_cash), 1.0) denominator floor (zero-base firings: 0 of 8, so reading (b) from the earlier audit is refuted). It is temporal: this overlay does not compare one quarter against the previous one, it compares the ANNUAL year end against the latest reported quarter, and across those fixtures the two are 91 to 273 days apart (AAPL 2025-09-27 -> 2026-06-27 = 273, MU and V also 273, COST 252, MELI and U96_SI 181, 09988_HK and BABA 91). Three quarters of balance-sheet drift is not a step change, it is a baseline the annual figure simply predates.

EXPECTED MOVES, NAMED BEFORE REGENERATING AND CONFIRMED AFTERWARD BY AN INDEPENDENT KEY-BY-KEY DIFF OF THE WHOLE SNAPSHOT DOCUMENT (scratchpad/diff_golden_snapshots.py, 14 fixtures and 3192 projection keys compared): NUMERIC LEAVES THAT MOVED = 0, and total changed keys = 7, of which 3 are _meta bookkeeping (generated_at, reason, regenerated_at_commit, which move on every regeneration by construction) and 4 are the MELI leaves named below. The other 13 fixtures are byte-identical, which also confirms that the industry-map change shipped alongside this one is golden-neutral: none of the names it re-routes is a golden fixture. All fourteen base intrinsic values are byte-identical and no weight, method leg, wacc, fx_rate, net_debt, shares_outstanding, gate metric on any other fixture, or 12m target moved anywhere. This is a disclosure-only flag: applied stays False, neither threshold aborts the overlay or reverts to stale annual data, and the denominator stays exactly max(abs(a_net_cash), 1.0) -- the annual figure alone, floored at one currency unit, with no second materiality floor added.

NON-NUMERIC LEAVES CHANGED = 4, ON ONE FIXTURE ONLY: MELI. (1) projection.gate_metrics LOSES 'balance_sheet_quarterly_step_change'; (2) projection.scenarios.base.forward_flags, (3) .bear.forward_flags and (4) .bull.forward_flags each LOSE the single step-change prose line. The three scenario lists move together because the lift appends to ticker_forward_flags, which the scenario loop copies per scenario -- the same channel the balance-sheet overlay prose already travels on.

WHY MELI AND ONLY MELI, from the measured ratio of every applied overlay rather than from a guess: MELI's net cash moves from -5.093bn at the 2025-12-31 year end to -7.446bn at 2026-06-30 -- the position got worse by 2.353bn, it did not improve -- a ratio of 0.4620 over a 181-day gap. It is the ONLY one of the eight that sits between the two thresholds, so it is the only one the ruling silences. Those two figures are corroborated by two independent sources that agree: the probe reconstructs them by calling _net_debt_net_of_investments itself (annual_net_cash -5093000000.0, quarterly_net_cash -7446000000.0), and the engine independently formatted "net cash -5.1bn at the 2025-12-31 year end -> -7.4bn at 2026-06-30 (+46%)" into MELI's forward_flags in the snapshot this regeneration replaces. That agreement matters more than it looks, because MELI is the one row where the probe's self-check against the engine's own record CANNOT run -- the engine writes no record when the flag does not fire -- so the reconstruction of a quiet overlay's ratio is otherwise unverified. The other seven clear 0.50 and still fire: 09988_HK 0.6401 (91-day gap, so it is measured against the STRICT 0.25 and the ruling never touched it), AAPL 0.6202 (273), BABA 0.6416 (91, strict, untouched), COST 1.3235 (252), MU 4.9521 (273), U96_SI 0.6309 (181), V 1.2920 (273). The remaining six fixtures (02888_HK, BN4_SI, C38U_SI, D05_SI, FCX, SCHW) are quiet because their overlay did not substitute a period.

THE INSTRUMENT, AND WHY ITS QUIET-ROW NUMBER IS TRUSTWORTHY. scratchpad/probe_stepchange_golden.py replays each fixture in its own subprocess (a shared process leaks ~ten process-lifetime caches and has previously reported BN4_SI at +26.54% where the truth is +0.00%). It was extended for this ruling to reconstruct delta_ratio for overlays that did NOT fire, which is the case whose ratio is otherwise invisible -- "MELI dropped out" is not a verified claim unless the number that dropped out is measured. The reconstruction calls the same _net_debt_net_of_investments the engine does, on the row before and after the field copy, and then checks itself against the engine's own published record on every fixture that fired: recon_matches_engine=True on 7 of 7. It also asserts the engine's fired set equals the resolved-threshold prediction (both ['09988.HK','AAPL','BABA','COST','MU','U96.SI','V']) and fails if the ruling GAINED any firing, which a strictly looser threshold cannot do. Result: would fire at a flat 0.25 = 8, fires at the resolved threshold = 7, dropped = [MELI], gained = [], zero-base firings = 0, all eight replays rc=0, PROBE_RC=0.

WHAT DELIBERATELY DID NOT MOVE, AND WHY THAT IS A DECISION RATHER THAN AN OMISSION. The ticker_forward_flags prose line the lift appends was left BYTE-IDENTICAL. It never named a threshold -- it reports the two net-cash figures, the ratio, "source ccy pre-FX", and "quarterly override APPLIED, disclosure only, no gate fired" -- so there was no stale number in it to correct, and the only reason to touch it would have been to add the resolved threshold. That was measured and rejected: golden_replay.py projects scenarios through _SCENARIO_KEYS, which INCLUDES 'forward_flags', so editing that sentence would have moved golden leaves on all seven surviving fixtures instead of one and blown past this named move set, for a number that is already carried where a reader of the audit payload can see it. For MELI the line now disappears entirely rather than changing wording, because the append sits inside the `if _bs_step:` branch and no record is written when the ratio does not clear the resolved threshold -- which is exactly the move named above.

WHAT THE GATE PAYLOAD GAINED AT ZERO BASELINE COST. The record gained two keys, period_delta_days and threshold_used, and its basis prose now interpolates the RESOLVED threshold ("above the {_th_used:.0%} structural step-change threshold") instead of hardcoding 25%, which under the conditional would have become an outright false statement in a published payload the first time a wide gap applied. golden_replay.py projects gate_evaluations down to sorted metric NAMES, so basis, raw_input_path_a, gated_output_path_b and both new keys are golden-invisible: the payload became self-describing without moving this baseline. period_delta_days and threshold_used are MY additions, not among the owner's six keys, and are disclosed as such in the source comment and in the tests: without threshold_used a reader sees delta_ratio 0.4620 and cannot tell whether it fired against 0.25 or was measured against 0.50 and dropped, and period_delta_days is the input that chose it, so the choice is auditable from the record rather than inferred from source. currency_basis remains the earlier disclosed seventh key; the owner's six keep their exact names and values. On an unparseable period the helper returns None and the caller falls back to the STRICTER 0.25 -- also mine, and the safe direction for a disclosure-only flag, since over-reporting ordinary drift is recoverable and silently dropping a structural move is not. That fallback is pinned by test, including that the overlay still applied, so the test cannot pass vacuously against the staleness guard.
...
## 2026-09-19T06:28:24+00:00

- regenerated at HEAD: `4f2062f`
- fixtures recorded at: `30b26702d3c786e1835dd8e5cc629191e3d75c95`
- tickers: 14
- tolerance: ±5% on numeric leaves
- reason: Two-tier valuation (owner decision 2026-09-19). Tier 1: fundamental IV = DCF + peer-median multiples, the quality x risk x commodity composite no longer multiplies the IV (composite_applied -> 1.0 everywhere). Tier 2: one 12m target rule for every name = spot + capture x (IV_premium - spot), capture unchanged (20-35%, +15pp unanimous), IV_premium = IV with the composite applied to the multiples leg only as a premium bounded by the peer interquartile range of the anchor multiple (fixtures carry no quartiles, so premium = 1.0x here). Forward-multiple / SOTP / normalised convergence / bank P/B / REIT P/FFO targets are kept as pt_bridge.cross_checks; the divergence guard and IV band are skipped under the unified rule. Unweighted legs published as cross_check_methods. Named moves (base IV; base 12m): COST 1324.57->816.88 (composite 1.78); MELI 5578.42->4007.19 (1.85); MU 293.14->182.97 (1.63); 02888_HK 284.38->258.53 (1.10); D05_SI 47.89->43.53 (1.10); 09988_HK 160.84->156.06, BABA 193.13->187.48 (1.036); BN4_SI 5.20->5.90 (0.88 discount removed). IV unchanged where composite was 1.0: AAPL C38U_SI FCX SCHW U96_SI V. Targets now lie between spot and IV on every name: FCX 24.89->48.02 (was below its own IV 26.69), SCHW 44.41->96.46 (IV 75.41), AAPL 257.66->275.57, C38U_SI 1.93->2.10, D05_SI 53.55->59.59, MU 439.71->555.29, 09988_HK 108.49->131.13, COST 1112.96->871.79, MELI 3141.26->2591.33, 02888_HK 257.69->240.64, BABA 151.25->148.43, BN4_SI 8.15->8.50; U96_SI and V unchanged.

## 2026-09-19T07:17:08+00:00

- regenerated at HEAD: `fdac4db`
- fixtures recorded at: `30b26702d3c786e1835dd8e5cc629191e3d75c95`
- tickers: 14
- tolerance: ±5% on numeric leaves
...
## 2026-09-19T10:31:17+00:00

- regenerated at HEAD: `1dcbe42`
- fixtures recorded at: `30b26702d3c786e1835dd8e5cc629191e3d75c95`
- tickers: 14
- tolerance: ±5% on numeric leaves
...
## 2026-09-26T06:51:15+00:00

- regenerated at HEAD: `af9f1e1`
- fixtures recorded at: `30b26702d3c786e1835dd8e5cc629191e3d75c95`
- tickers: 14
- tolerance: ±5% on numeric leaves

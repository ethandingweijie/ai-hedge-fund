...
def test_admin_trigger_infers_drop_from_negative_pct(client):
    r = client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11")
...
def test_admin_trigger_infers_pump_from_positive_pct(client):
    r = client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=NVDA&pct=0.12")
    assert r.status_code == 200
    body = r.json()
...
def test_admin_trigger_explicit_direction_overrides(client):
    ...
    r = client.post(
        f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11&direction=DROP"
    )
    assert r.status_code == 200
...
def test_admin_trigger_invalid_direction_rejected(client):
    r = client.post(
        f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11&direction=NEUTRAL"
    )
...
def test_admin_trigger_explicit_price_used(client):
    r = client.post(
        f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11&price=85.50"
    )
    assert r.status_code == 200
...
def test_admin_trigger_blocks_when_in_cooldown(client):
    ...
    r1 = client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11")
    assert r1.json()["fired"] is True
    r2 = client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.12")
...
def test_admin_trigger_force_bypasses_cooldown(client):
    ...
    assert r1.json()["fired"] is True
    r2 = client.post(
        f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11&force=true"
    )
...
def test_admin_trigger_continues_when_slack_post_raises(client, monkeypatch):
    ...
    r2 = client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11")
    assert r2.json()["fired"] is False
    assert r2.json()["eligibility_reason"].startswith("in_cooldown")
...
def test_list_alerts_returns_recent_alerts_with_report(client):
    ...
    client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11")
    r = client.get("/api/dd-alerts")
    body = r.json()
    assert len(body) == 1
    item = body[0]
...
def test_list_alerts_filter_by_direction(client):
    client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11")
    client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=NVDA&pct=+0.12")
...
def test_list_alerts_limit_caps_result_count(client):
    for tk in ("AAA", "BBB", "CCC", "DDD"):
        client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker={tk}&pct=-0.11")
...
def test_digest_today_aggregates_drops_and_pumps(client):
    client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11")
    client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=CRWD&pct=-0.15")
    client.post(f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=NVDA&pct=+0.12")
...
def test_get_detail_returns_full_record(client):
    trigger = client.post(
        f"/admin/dd-trigger?secret={ADMIN_SECRET}&ticker=PEGA&pct=-0.11"
    ).json()

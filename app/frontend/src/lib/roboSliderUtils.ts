 ...
 * roboSliderUtils.ts
 * ===================
 * Linked-slider utility for the Robo Strategy questionnaire: when one weight
 * changes, the others adjust proportionally so the total always sums to
 * 100%. Ported near-verbatim from the "Robo Strategy" prototype's
 * src/lib/sliderUtils.ts — dependency-free pure math, no changes needed.
 */
export const TARGET_TOTAL = 100;

/**
 * Adjust a record of weights so that `changedKey` becomes `newWeight` and
 * all other keys are scaled proportionally. The result always sums to
 * exactly `target` (default 100).
 */

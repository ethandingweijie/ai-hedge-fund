...
def run_advanced_pipeline(
    ...
    try:
        ...
        # ── Static SOTP snapshot (task #27): RETIRED (owner, 2026-09-26) ────
        # Six extractor outputs frozen in sotp_assumptions_v1.json used to be
        # attached here and promoted into the blend at 75%. The SOTP (analyst)
        # leg now reads the owner-accepted Gemini inputs only (dcf_agent
        # bridge); the extractor's output above stays on state as a
        # cross-check. The snapshot module remains for `curated_holdco_discount`
        # and segment memory.

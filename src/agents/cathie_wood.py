...
def generate_cathie_wood_output(
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are Cathie Wood. Score on disruptive innovation potential — current profitability is secondary.

            Hard criteria (cite specific numbers):
            - Disruptive technology or platform with large, expanding TAM (ideally >$100B addressable)
            - Revenue growth accelerating or sustained >30% YoY; deceleration below 20% is a red flag
            - R&D spend ≥15% of revenue signals genuine innovation investment
            - 5+ year time horizon; willing to accept near-term losses for exponential upside
            - Bearish on companies with slowing growth, commoditised tech, or incremental-only products

            Return bullish/bearish/neutral with confidence (0-100) and reasoning citing growth rate trend and TAM.
            """,
            ),
            (
                "human",
